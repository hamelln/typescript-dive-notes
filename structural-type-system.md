...
`Swift` 같은 명목적 타입 언어는 이런 케이스에 `Foodable protocol`을 `extension`으로 확장해서 사용합니다. 당연한 처리입니다. 하지만 좀 더 직관적으로 생각해보죠! 햄버거가 비록 `Food` 타입은 아니지만 탄수화물, 단백질, 지방 정보가 있으니 칼로리 계산을 허용하면 훌륭하지 않을까요?  
`TypeScript`는 별도의 처리 없이 바로 칼로리 계산을 허용합니다! 햄버거는 탄수화물, 단백질, 지방이라는 Food의 '속성'을 전부 갖췄기 때문입니다. 심지어 버거 브랜드라는 **잉여 속성도 관대하게 허용**합니다. 아까까지 허술하기만 보였던 시스템이 유연함으로 작용합니다. 이런 특징은 다형성과 연계됩니다.
...
하지만 융통성에 치우치면 타입 체크는 허술해집니다. 위에서도 봤듯이, 이런 동작은 유연함과 허술함을 동시에 가진 양날의 검이기 때문입니다. `TypeScript`에는 이를 조심하는 기술이 몇 가지 있습니다. 그 중에 하나가 객체 리터럴입니다. 아래를 봅시다.
...
`TypeScript`는 객체 리터럴을 $\textcolor{#3498DB}{\textsf{fresh한 객체}}$라고 분류하고, 요구하는 속성보다 많을 시 `Error`를 냅니다.  
즉, 인자를 객체 리터럴로 보낼 경우 오로지 `carbohydrates`, `fat`, `protein` 세 가지 속성만 적어야 합니다.
